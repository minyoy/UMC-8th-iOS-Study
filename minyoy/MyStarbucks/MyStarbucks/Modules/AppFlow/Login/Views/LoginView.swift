...
struct LoginView: View {
    ...
    private var middleGroup: some View {
        VStack(alignment: .leading, spacing: 47) {
            ...
            Button(action: {
                print("로컬 로그인 버튼")
                if viewModel.email == savedEmail &&
                    viewModel.pwd == savedPassword {
    ...
    private var bottomGroup: some View {
        VStack(alignment: .center, spacing: 19) {
            Button(action: {
                container.navigationRouter.push(.signup)
            }, label: {
                Text("이메일로 회원가입하기")
                ...
                if (UserApi.isKakaoTalkLoginAvailable()) {
                    UserApi.shared.loginWithKakaoTalk {(oauthToken, error) in
                        if let error = error {
                            ...
                            KeychainService.shared.saveToken(accessToken: oauthToken.accessToken, refreshToken: oauthToken.refreshToken)
